...
function ScannerScreen({ onCancel, onLookup, mode, setMode }) {
  ...
  const containerId = 'gz-camera-' + (typeof crypto !== 'undefined' && crypto.randomUUID ? crypto.randomUUID().slice(0,8) : Math.random().toString(36).slice(2,10));
  ...
  useEffect(() => {
    ...
    const start = async () => {
      ...
      try {
        ...
        if (/permission|notallowed|denied/i.test(name)) {
          setCamPhase('denied');
        } else if (/notfound|nocamera|devicesnotfound/i.test(name)) {
          setCamPhase('nocam');
        } else {
          setCamPhase('error');
          setCamError('No se pudo iniciar la cámara. Usa el modo Manual.');
        }
      }
    };
    start();
