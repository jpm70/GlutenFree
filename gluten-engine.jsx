...
// so the verdict logic stays in one place.
function analyzeText(rawText, opts = {}) {
  const text = String(rawText || '').replace(/\s+/g, ' ').trim();
  if (!text) return null;

  // Try to isolate the ingredient list — many EU labels include "Ingredientes:" / "Ingredients:"
  // followed by the actual list. If found, use that section; otherwise use the whole text.
  let ingredientsText = text;
  const m = text.match(/ingredient(?:es|s|en|i)\s*[:.\-]\s*([\s\S]{4,800})/i);
  if (m) ingredientsText = m[1].trim();

  // Stop at the next obvious section header (Información nutricional / Nutritional / etc.)
  ...
  const product = {
    product_name: opts.name || 'Etiqueta escaneada',
    brands: opts.brand || '',
  ...
  return {
    ...
    source: 'ocr',
    ocrText: text,
    ocrIngredients: ingredientsText,
  };
}
